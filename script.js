...
if(!u){
    document.getElementById("perfilMensaje").innerText = "No hay usuario cargado";
    return;
}

// llenar datos
document.getElementById("perfilNombreTexto").innerText = u.nombre || "Usuario";
...
if(!u){
    document.getElementById("perfilMensaje").innerText = "Error: no hay usuario";
    return;
}

// obtener valores
let correo = document.getElementById("perfilCorreo").value.trim();
let telefono = document.getElementById("perfilTelefono").value.trim();
let nuevaPass = document.getElementById("perfilPassword").value.trim();
let confirmarPass = document.getElementById("perfilPassword2").value.trim();

// actualizar SIEMPRE estos
u.correo = correo;
u.telefono = telefono;

// 🔥 SOLO valida contraseña si AMBOS están llenos
if(nuevaPass !== "" && confirmarPass !== ""){

    if(nuevaPass !== confirmarPass){
        document.getElementById("perfilMensaje").innerText = "Las contraseñas no coinciden ❌";
        return;
    }
...
// mensaje
document.getElementById("perfilMensaje").innerText = "Datos actualizados correctamente ✅";
...
function descargarReceta(nombre, doctor, dosis, instrucciones, fecha){
...
let a = document.createElement("a");
a.href = url;
a.download = nombre.replace(/\s/g, "_") + ".txt";
a.click();
...
let mensajes = JSON.parse(localStorage.getItem("chatMensajes")) || [];
